...
public class AbilitySystem
{
    ...
    public bool ExecuteAbility(IServerPlayer player, string abilityId)
    {
        ...
        if (!ability.CanExecute(player, _sapi, out var failureReason))
        {
        ...
        try
        {
            success = ability.Execute(player, _sapi, BuffManager);
        }
        catch (Exception ex)
        {
    ...
    /// </summary>
    public IEnumerable<Ability> GetPlayerAbilities(IServerPlayer player)
    {
        var playerData = _playerDataManager.GetOrCreatePlayerData(player);
        if (!playerData.HasDeity()) return Enumerable.Empty<Ability>();
