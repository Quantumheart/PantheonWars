...
public class PerkEffectSystem : IPerkEffectSystem
{
    ...
    public Dictionary<string, float> GetPlayerStatModifiers(string playerUID)
    {
        ...
        foreach (var perkId in unlockedPerkIds)
        {
            ...
            if (perk != null && perk.Kind == PerkKind.Player) CombineModifiers(modifiers, perk.StatModifiers);
        }
    ...
    public Dictionary<string, float> GetReligionStatModifiers(string religionUID)
    {
        ...
        {
            var perk = _perkRegistry.GetPerk(perkId);
            if (perk != null && perk.Kind == PerkKind.Religion) CombineModifiers(modifiers, perk.StatModifiers);
        }
    ...
    public void RefreshPlayerPerks(string playerUID)
    {
        ...
        if (player != null) ApplyPerksToPlayer(player);

        _sapi.Logger.Debug(
            $"{SystemConstants.LogPrefix} {string.Format(SystemConstants.SuccessRefreshedPerksFormat, playerUID)}");
    }
    ...
    public void RefreshReligionPerks(string religionUID)
    {
        ...
        // Refresh all members
        foreach (var memberUID in religion.MemberUIDs) RefreshPlayerPerks(memberUID);

        _sapi.Logger.Debug(
            $"{SystemConstants.LogPrefix} {string.Format(SystemConstants.SuccessRefreshedReligionPerksFormat, religion.ReligionName, religion.MemberUIDs.Count)}");
    }
    ...
    public (List<Perk> playerPerks, List<Perk> religionPerks) GetActivePerks(string playerUID)
    {
        ...
        {
            var perk = _perkRegistry.GetPerk(perkId);
            if (perk != null) playerPerks.Add(perk);
        }
        ...
        if (playerData.ReligionUID != null)
        {
            ...
            if (religion != null)
            {
                ...
                {
                    var perk = _perkRegistry.GetPerk(perkId);
                    if (perk != null) religionPerks.Add(perk);
                }
            }
        }
