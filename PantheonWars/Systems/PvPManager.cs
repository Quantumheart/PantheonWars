    ...
    private void ProcessPvPKill(IServerPlayer attacker, IServerPlayer victim)
    {
        ...
        if (attackerReligion == null)
        {
            _sapi.Logger.Warning(
                $"[PantheonWars] Attacker {attacker.PlayerName} has invalid religion UID: {attackerData.ReligionUID}");
            return;
        }
        ...
        _sapi.Logger.Debug(
            $"[PantheonWars] {attacker.PlayerName} earned {favorReward} favor and their religion earned {prestigeReward} prestige for killing {victim.PlayerName}");
    }
