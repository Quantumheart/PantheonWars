...
public class PerkRegistry : IPerkRegistry
{
    ...
    public void Initialize()
    {
        ...
        foreach (var perk in allPerks) RegisterPerk(perk);
        ...
        Perk? perk)
    {
        ...
        if (perk == null) return (false, "Perk not found");
        ...
        if (perk.Kind == PerkKind.Player)
        {
            ...
            if (playerData.IsPerkUnlocked(perk.PerkId)) return (false, "Perk already unlocked");
            ...
            foreach (var prereqId in perk.PrerequisitePerks)
                if (!playerData.IsPerkUnlocked(prereqId))
                {
                    ...
                    var prereqName = prereqPerk?.Name ?? prereqId;
        ...
        // Check if already unlocked
        if (religionData.UnlockedPerks.TryGetValue(perk.PerkId, out var unlocked) && unlocked)
        ...
        // Check prerequisites
        foreach (var prereqId in perk.PrerequisitePerks)
            if (!religionData.UnlockedPerks.TryGetValue(prereqId, out var prereqUnlocked) || !prereqUnlocked)
            {
                var prereqPerk = GetPerk(prereqId);
                var prereqName = prereqPerk?.Name ?? prereqId;
                return (false, $"Requires prerequisite perk: {prereqName}");
