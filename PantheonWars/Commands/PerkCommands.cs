...
{
    private readonly IPerkEffectSystem _perkEffectSystem =
        perkEffectSystem ?? throw new ArgumentNullException($"{nameof(sapi)}");
    ...
    internal TextCommandResult OnPerksList(TextCommandCallingArgs args)
    {
        ...
        {
            var status = playerData.IsPerkUnlocked(perk.PerkId) ? FormatStringConstants.LabelUnlocked : "";
            var requiredRank = (FavorRank)perk.RequiredFavorRank;
        ...
        {
            var unlocked = religion?.UnlockedPerks.TryGetValue(perk.PerkId, out var u) == true && u;
            var status = unlocked ? FormatStringConstants.LabelUnlocked : "";
            var requiredRank = (PrestigeRank)perk.RequiredPrestigeRank;
            sb.AppendLine($"{perk.Name} {status}");
            sb.AppendLine(string.Format(FormatStringConstants.FormatPerkId, perk.PerkId));
            sb.AppendLine(string.Format(FormatStringConstants.FormatRequiredRank, requiredRank));
            sb.AppendLine(string.Format(FormatStringConstants.FormatDescription, perk.Description));
    ...
    internal TextCommandResult OnPerksPlayer(TextCommandCallingArgs args)
    {
        ...
        if (playerPerks.Count == 0) return TextCommandResult.Success(InfoMessageConstants.InfoNoPlayerPerks);
    ...
    internal TextCommandResult OnPerksReligion(TextCommandCallingArgs args)
    {
        ...
        if (religionPerks.Count == 0) return TextCommandResult.Success(InfoMessageConstants.InfoNoReligionPerks);
    ...
    internal TextCommandResult OnPerksInfo(TextCommandCallingArgs args)
    {
        ...
        if (string.IsNullOrEmpty(perkId)) return TextCommandResult.Error(UsageMessageConstants.UsagePerksInfo);
    ...
    internal TextCommandResult OnPerksTree(TextCommandCallingArgs args)
    {
        ...
        var perkKind = type == FormatStringConstants.TypeReligion
            ? PerkKind.Religion
            : PerkKind.Player;
        ...
        if (perkKind == PerkKind.Player)
            foreach (FavorRank rank in Enum.GetValues(typeof(FavorRank)))
            {
                ...
                {
                    var unlocked = playerData.IsPerkUnlocked(perk.PerkId);
                    var status = unlocked
    ...
    internal TextCommandResult OnPerksUnlock(TextCommandCallingArgs args)
    {
        ...
        if (player == null) return TextCommandResult.Error(ErrorMessageConstants.ErrorPlayerNotFound);
        var perkId = args[0] as string;
        if (string.IsNullOrEmpty(perkId)) return TextCommandResult.Error(UsageMessageConstants.UsagePerksUnlock);
        ...
        if (perk.Kind == PerkKind.Player)
        {
            ...
            var success = _playerReligionDataManager.UnlockPlayerPerk(player.PlayerUID, perkId);
