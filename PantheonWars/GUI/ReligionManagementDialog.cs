...
namespace PantheonWars.GUI
{
    ...
    public class ReligionManagementDialog : GuiDialog
    {
        ...
        private CreateReligionDialog _createDialog;
        ...
        private List<ReligionListResponsePacket.ReligionInfo> _availableReligions = new List<ReligionListResponsePacket.ReligionInfo>();
        ...
        private void ComposeDialog()
        {
            ...
            composer.AddSmallButton("Create New", OnCreateReligionClicked, createButtonBounds, EnumButtonStyle.Normal, "createButton");
            ...
            else if (_currentTab == "my_religion")
            {
                ComposeMyReligionTab(composer, tabContentBounds);
            }
        ...
        private void ComposeBrowseTab(GuiComposer composer, ElementBounds bounds)
        {
            ...
            var deityNames = new string[] { "All Deities" }
            ...
            var listBounds = ElementBounds.Fixed(bounds.fixedX, bounds.fixedY + 40, bounds.fixedWidth, bounds.fixedHeight - 40);
            ...
            {
                var noReligionsTextBounds = ElementBounds.Fixed(listBounds.fixedX + 10, listBounds.fixedY + 10, listBounds.fixedWidth - 20, 30);
                composer.AddStaticText("No religions available. Create one!", CairoFont.WhiteSmallText(), noReligionsTextBounds);
            }
            else
            {
                double yPos = listBounds.fixedY + 10;
        ...
        private void AddReligionListItem(GuiComposer composer, ReligionListResponsePacket.ReligionInfo religion, double x, double y, double width)
        {
        ...
        private void ComposeMyReligionTab(GuiComposer composer, ElementBounds bounds)
        {
            ...
            {
                var noReligionBounds = ElementBounds.Fixed(bounds.fixedX + 10, bounds.fixedY + 10, bounds.fixedWidth - 20, 30);
                composer.AddStaticText("You are not in a religion. Browse or create one!", CairoFont.WhiteSmallText(), noReligionBounds);
                return;
            }

            var religion = _playerReligionInfo;
            double yPos = bounds.fixedY + 10;
            ...
            var prestigeRankEnum = Enum.TryParse<PrestigeRank>(religion.PrestigeRank, out var rank) ? rank : PrestigeRank.Fledgling;
            composer.AddStaticText(
            ...
            composer.AddStaticText("Members:", CairoFont.WhiteSmallText().WithWeight(FontWeight.Bold), memberListLabelBounds);
            yPos += 25;
            ...
            double memberYPos = memberListBounds.fixedY + 5;
            foreach (var member in religion.Members)
            {
                AddMemberListItem(composer, member, memberListBounds.fixedX + 10, memberYPos, memberListBounds.fixedWidth - 20);
                memberYPos += 30;
            }
            ...
            double buttonY = bounds.fixedY + bounds.fixedHeight - 40;
            double buttonX = bounds.fixedX + 10;
            ...
            if (religion.IsFounder)
            {
                ...
                composer.AddSmallButton("Disband", OnDisbandReligionClicked, disbandButtonBounds, EnumButtonStyle.Normal);
            }
            ...
            composer.AddSmallButton("Leave Religion", OnLeaveReligionClicked, leaveButtonBounds, EnumButtonStyle.Normal);
        }

        private void AddMemberListItem(GuiComposer composer, PlayerReligionInfoResponsePacket.MemberInfo member, double x, double y, double width)
        {
            ...
            if (_playerReligionInfo != null && _playerReligionInfo.IsFounder && !member.IsFounder && member.PlayerUID != _capi.World.Player.PlayerUID)
            {
        ...
        private bool OnEditDescriptionClicked()
        {
            if (_playerReligionInfo != null && _playerReligionInfo.HasReligion)
            {
                ...
                _editDescriptionDialog = new EditDescriptionDialog(_capi, _channel, _playerReligionInfo.ReligionUID, _playerReligionInfo.Description);
