...
namespace PantheonWars.GUI.UI.Renderers
{
    ...
    {
        // Color constants
        private static readonly Vector4 ColorDivider = new Vector4(0.573f, 0.502f, 0.416f, 1.0f); // #92806a
        private static readonly Vector4 ColorLabel = new Vector4(0.996f, 0.682f, 0.204f, 1.0f);   // #feae34 gold
            ...
            ref string? hoveringPerkId)
        {
            ...
            float panelWidth = (width - dividerWidth) / 2f;
            float treeAreaHeight = height - labelHeight;
            ...
            float treeY = leftY + labelHeight;
            float playerScrollX = manager.PlayerTreeScrollX;
            float playerScrollY = manager.PlayerTreeScrollY;
            DrawTreePanel(
            ...
            float rightX = dividerX + dividerWidth;
            float rightY = y;
            ...
            // Draw tree area (use local variables for scroll offsets)
            float religionScrollX = manager.ReligionTreeScrollX;
            float religionScrollY = manager.ReligionTreeScrollY;
            DrawTreePanel(
                manager, api, drawList,
        ...
        private static void DrawTreePanel(
            ...
            float x, float y, float width, float height,
            System.Collections.Generic.Dictionary<string, PerkNodeState> perkStates,
            ...
            ref string? hoveringPerkId)
        {
            ...
            float totalHeight = PerkTreeLayout.GetTotalHeight(perkStates);
            float totalWidth = PerkTreeLayout.GetTotalWidth(perkStates);
            ...
            float localMouseX = mousePos.X - childWindowPos.X;
            float localMouseY = mousePos.Y - childWindowPos.Y;
            ...
            foreach (var state in perkStates.Values)
            {
                bool isSelected = manager.SelectedPerkId == state.Perk.PerkId;
                bool isHovering = PerkNodeRenderer.DrawNode(
                    state, api,
                    childWindowPos.X + padding, childWindowPos.Y + padding,
