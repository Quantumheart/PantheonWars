...
{
    /// <summary>
    /// Renders individual perk nodes in the tree
    /// Displays states: locked, unlockable (glowing), unlocked (gold)
    /// </summary>
    internal static class PerkNodeRenderer
    {
        private static readonly Vector4 ColorLocked = new Vector4(0.573f, 0.502f, 0.416f, 1.0f);     // #92806a grey
        private static readonly Vector4 ColorUnlockable = new Vector4(0.478f, 0.776f, 0.184f, 1.0f); // #7ac62f lime
        private static readonly Vector4 ColorUnlocked = new Vector4(0.996f, 0.682f, 0.204f, 1.0f);   // #feae34 gold
        private static readonly Vector4 ColorSelected = new Vector4(1.0f, 1.0f, 1.0f, 1.0f);         // White border
        private static readonly Vector4 ColorHover = new Vector4(0.8f, 0.8f, 1.0f, 1.0f);            // Light blue tint

        /// <summary>
        /// Draw a single perk node
        /// </summary>
        /// <param name="state">Perk node state</param>
            ...
            bool isSelected)
        {
            ...
            float screenX = state.PositionX + offsetX;
            float screenY = state.PositionY + offsetY;

            // Check if mouse is hovering
            bool isHovering = mouseX >= screenX && mouseX <= screenX + state.Width &&
            ...
            float radius = state.Width / 2;

            Vector4 nodeColor = state.VisualState switch
            {
                PerkNodeVisualState.Locked => ColorLocked,
                PerkNodeVisualState.Unlockable => ColorUnlockable,
                PerkNodeVisualState.Unlocked => ColorUnlocked,
                _ => ColorLocked
            };
            ...
            if (isHovering)
            {
                nodeColor = Vector4.Lerp(nodeColor, ColorHover, 0.3f);
            }
            ...
            // Draw border
            Vector4 borderColor = isSelected ? ColorSelected : nodeColor * 0.7f;
            ...
            {
                perkName = perkName.Substring(0, 10) + "..";
            }
            ...
            Vector4 textColor = state.VisualState == PerkNodeVisualState.Unlocked
        ...
        public static void DrawConnectionLine(
            PerkNodeState fromNode,
            PerkNodeState toNode,
            float offsetX, float offsetY)
        {
            var drawList = ImGui.GetWindowDrawList();
