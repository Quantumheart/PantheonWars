    ...
    public static class PerkTreeLayout
    {
        ...
        public static void CalculateLayout(Dictionary<string, PerkNodeState> perkStates, float containerWidth)
        {
            ...
            foreach (var tierGroup in nodesByTier)
            {
                ...
                int nodeCount = nodesInTier.Count;

                // Calculate horizontal layout for this tier
                float totalWidth = (nodeCount * NodeWidth) + ((nodeCount - 1) * HorizontalSpacing);
                float startX = LeftPadding + (containerWidth - totalWidth - LeftPadding * 2) / 2;

                // If total width exceeds container, use left alignment with smaller spacing
                if (totalWidth > containerWidth - LeftPadding * 2)
                {
                    startX = LeftPadding;
                    float availableWidth = containerWidth - LeftPadding * 2 - (nodeCount * NodeWidth);
                    float adjustedSpacing = nodeCount > 1 ? availableWidth / (nodeCount - 1) : 0;
        ...
        private static void AssignTiers(Dictionary<string, PerkNodeState> perkStates)
        {
            ...
            {
                CalculateTier(state, stateMap, new HashSet<string>());
            }
        }
        ...
        /// </summary>
        private static int CalculateTier(PerkNodeState node, Dictionary<string, PerkNodeState> stateMap, HashSet<string> visiting)
        {
            ...
            foreach (var prereqId in node.Perk.PrerequisitePerks)
            {
                ...
                {
                    int prereqTier = CalculateTier(prereqState, stateMap, visiting);
        ...
        public static float GetTotalHeight(Dictionary<string, PerkNodeState> perkStates)
        {
            ...
            int maxTier = perkStates.Values.Max(node => node.Tier);
        ...
        public static float GetTotalWidth(Dictionary<string, PerkNodeState> perkStates)
        {
            ...
            int maxNodesInTier = perkStates.Values
                .GroupBy(node => node.Tier)
