...
namespace PantheonWars.GUI
{
    ...
    public class PerkDialog : ModSystem
    {
        ...
        private void LoadTestData()
        {
            ...
            // Create test perks
            var testPlayerPerks = new System.Collections.Generic.List<Models.Perk>
            {
                new Models.Perk("khoras_warriors_resolve", "Warrior's Resolve", DeityType.Khoras)
                {
                    Kind = Models.Enum.PerkKind.Player,
                    Category = Models.Enum.PerkCategory.Combat,
                    Description = "Increases melee damage and health in combat.",
                    RequiredFavorRank = 0,
                    StatModifiers = new System.Collections.Generic.Dictionary<string, float>
                    {
                        { "meleeDamage", 0.1f },
                        { "maxhealthExtraMultiplier", 0.1f }
                    }
                },
                new Models.Perk("khoras_bloodlust", "Bloodlust", DeityType.Khoras)
                {
                    Kind = Models.Enum.PerkKind.Player,
                    Category = Models.Enum.PerkCategory.Combat,
                    Description = "Gain attack speed after killing an enemy.",
                    RequiredFavorRank = 1,
                    PrerequisitePerks = new System.Collections.Generic.List<string> { "khoras_warriors_resolve" }
                }
            };

            var testReligionPerks = new System.Collections.Generic.List<Models.Perk>
            {
                new Models.Perk("khoras_war_banner", "War Banner", DeityType.Khoras)
                {
                    Kind = Models.Enum.PerkKind.Religion,
                    Category = Models.Enum.PerkCategory.Combat,
        ...
        private void DrawWindow()
        {
            ...
            int windowWidth = Math.Min(WindowBaseWidth, (int)window.OuterWidth - 128);
            int windowHeight = Math.Min(WindowBaseHeight, (int)window.OuterHeight - 128);
